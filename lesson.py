...
for symbol in original_text:
    ...
    if original_index_in_alphabet == -1: #т.к фаинд возврашает -1 если не нашёл
        encrypted_text += symbol
    
    else:
        encrypted_index_in_alphabet = (original_index_in_alphabet + encryption_key) % len(ALPHABET)

        encrypted_text += ALPHABET[encrypted_index_in_alphabet]

print(f"Зашифрованнный текст: {encrypted_text}")
